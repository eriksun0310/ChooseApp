...
const Roulette = () => {
  ...
  const renderWheelItems = () => {
    ...
    return awards.map((item, index) => {
      ...
      const textX =
        wheelRadius + (wheelRadius - 50) * Math.cos(currentRadian + RadianGap / 2);
      const textY =
        wheelRadius + (wheelRadius - 50) * Math.sin(currentRadian + RadianGap / 2);
      ...
      return (
        <G key={index}>
          ...
          <Text
            ...
            fontSize="16"
            textAnchor="middle"
            transform={`rotate(${(currentRadian - RadianGap / 2) * (180 / Math.PI)}, ${textX}, ${textY})`}
          >
            {item.name}
          </Text>
        </G>
      ...
      <View style={styles.wheelContainer}>
        ...
        >
          <Svg width="300" height="300" viewBox="0 0 300 300">
            <Circle cx="150" cy="150" r="140" fill="lightgray" stroke="black" strokeWidth="2" />
            {renderWheelItems()}
          </Svg>
        </Animated.View>
        <Svg width="300" height="300" viewBox="0 0 300 300" style={styles.indicatorContainer}>
          <Polygon points="145,0 155,0 150,20" fill="blue" />
        </Svg>
      </View>
